...
import gagak1 from "./assets/hewan/gagak 1.png";
import gagak2 from "./assets/hewan/gagak 2.png";
import gagakSound from "./assets/suara_hewan/gagak.mp3";
...
export default function Level4Drag({ onBackToGameCanvas }) {
  // Reset sessionStorage jika halaman di-refresh (hanya saat mount pertama)
  useEffect(() => {
    sessionStorage.removeItem("level4_completed");
  }, []);
  ...
  const [showGagak2, setShowGagak2] = useState(false);
  ...
  function handleRemoveDrop(slotIdx) {
    ...
    useEffect(() => {
      if (
        ...
        sessionStorage.setItem("level4_A_completed", "1");
        setShowWinModal(true);
        if (menangAudioRef.current) {
          menangAudioRef.current.currentTime = 0;
          menangAudioRef.current.play();
  ...
  return (
    <div className="levelA-drag-container">
      ...
      <div className="level-label-roww">
        <div className="level-label label-level">Level 4</div>
      ...
      <div className="hewan-panel" style={{ left: "23%", top: "150px" }}>
        <img
          src={showGagak2 ? gagak2 : hover ? gagak2 : gagak1}
          alt="Gagak"
          className={`hewan-img${hover ? " hewan-img-besar" : ""}`}
          onMouseEnter={handleMouseEnter}
          onMouseLeave={handleMouseLeave}
        />
        <audio ref={audioRef} src={gagakSound} />
      </div>
